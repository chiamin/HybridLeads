...
class BdGBasis {
 public:
  ...
  // en(i) is the energy in H_BdG = sum_i^N { en(i) gamma_i^dag gamma_i } -
  // sum_i^N { (1/2) en(i) }
  Real en(int k) const { return _ens(k - 1); }
  Real mu(int k) const {
    mycheck(k <= this->size(), "Out of range");
    return -2. * _H(k - 1, k - 1);
  }
...
//
// The particle-hole correspondence for the E_i and -E_i basis states is
// autoomatically satisfied for the non-zero-energy modes. However for
// zero-energy (Majorana) modes, since |phi_i> and S|phi_i> are degenerate,
...
Matrix symmetrice_zero_energy_modes(const Vector& ens, Matrix U,
  ...
  Vector v_pos(2 * N), v_neg(2 * N);
  for (int i = 0; i < N0; i++) {
    v_pos += w_pos(i) * states.at(i);
    v_neg += w_neg(i) * states.at(i);
  }
  mycheck(abs(e_pos - 1) < 1e-14 and abs(e_neg + 1) < 1e-14,
          "particle-hole eigenvalues error");
...
BdGBasis ::BdGBasis(const string& name, int L, Real t, Real mu, Real Delta)
  ...
  // ens(q) for q=0,1,...,2N-1 is the energy in descending order (highest to
  // lowest). The energy for q=0,...N-1 is positive
  ...
  if constexpr (!is_same_v<decltype(tmp), Real>) {
    cout << "The current version is only for unitary matrix of Real type"
         << endl;
  ...
  //      [ _v  _u* ]
  // Take only the positive-energy states, i.e. the first N columns, to define
  // _u and _v.
  int N = ens.size() / 2;
  _ens = Vector(N);
  _u = Matrix(N, N);
...
vector<tuple<int, auto, bool>> BdGBasis ::C_op(int i, bool dag) const {
  ...
  for (int k = 0; k < N; k++) {
    auto uk = _u(i - 1, k);
    auto vk = _v(i - 1, k);
    if (abs(uk) > 1e-14) k_coef_dag.emplace_back(k + 1, uk, false);
    if (abs(vk) > 1e-14) k_coef_dag.emplace_back(k + 1, iut::conj(vk), true);
  }
  // If Cdag
  if (dag) {
...
vector<tuple<int, auto, bool>> BdGBasis ::gamma_op(int k, bool dag) const {
  ...
  for (int i = 0; i < N; i++) {
    auto uk = _u(i, k - 1);
    auto vk = _v(i, k - 1);
    if (abs(uk) > 1e-14) i_coef_dag.emplace_back(i + 1, uk, true);
    if (abs(vk) > 1e-14) i_coef_dag.emplace_back(i + 1, vk, false);
  }
  // If gamma
  if (!dag) {
...
auto write(ostream& s, const BdGBasis& t) { t.write(s); }
auto read(istream& s, BdGBasis& t) { t.read(s); }
