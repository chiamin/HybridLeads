 ...
 public:
  ...
  MixedBasis(int N, int iL, int iR, int iC, Args const& args = Args::global()) {
    ...
    for (int j = 1; j <= N; ++j) {
      ...
      if (j == iC)
        sites.set(j, SpecialBosonSite({args, "SiteNumber=", j}));
      else
        sites.set(j, SpecialFermionSite(
                         {args, "SiteNumber=", j, "in_scatter", in_scatter}));
    }
    SiteSet::init(std::move(sites));
  }

  MixedBasis(int N, const vector<int>& scatter_sites, int iC,
  ...
  MixedBasis(IndexSet const& is, Args const& args = Args::global()) {
    ...
    for (auto j : range1(N)) {
      auto ii = is(j);
      mycheck(hasTags(ii, "Boson") or hasTags(ii, "Fermion"),
              "unknown site index");
      if (hasTags(ii, "Boson")) {
