...
void denmatSumDecomp(std::vector<MPS> const& psis, MPS& res,
                     std::vector<ITensor>& Bs, int b, Direction dir,
  ...
  auto [V1, S1, U1] = svd(Bs.front(), dir == Fromleft ? rightLinkIndex(res, b)
                                                      : leftLinkIndex(res, b));
  ...
  if (dim(mid) <= dim(commonIndex(U1, S1))) {
    ...
    if (numCenter == 1) {
      ...
      for (auto B = Bs.begin() + 1; B != Bs.end(); ++B, ++psi) {
        rho2 += prime(*B) *
                dag(prime(*B, dir == Fromleft ? rightLinkIndex(*psi, b)
                                              : leftLinkIndex(*psi, b)));
      }
      rho2.swapPrime(0, 1);
    } else {
    ...
    auto proj2 =
        toDense(delta(dag(mid), prime(mid))) - dag(U1) * prime(U1, mid);
    ...
    if (normPrho2P / normrho2 <
        1E-14)  // TODO: changed to calculate the trace will have less
                // complexity and have the same effect!
    {
      res.ref(b) = cmb * U1;
      if (!quiet) printfln("warning: at bond %d, not adding any new basis.", b);
    } else {
...
void addBasisWorker(std::vector<MPS> const& psis, MPS& res, Direction dir,
                    const Args& args = Args::global()) {
  int N = length(res);
  int nt = psis.size() + 1;

  if (dir == Fromleft) {
    if (orthoCenter(res) != 1) Error("OC need set to be 1");
    for (auto& psi : psis) {
      if (orthoCenter(psi) != 1) Error("OC need set to be 1");
    }
    ...
    if (orthoCenter(res) != N) Error("OC need set to be N");
    for (auto& psi : psis) {
      if (orthoCenter(psi) != N) Error("OC need set to be N");
    }
...
void addBasis(MPS& phi, const MPO& H, Real truncK, int maxDim,
  ...
  auto donormalize =
      args0.getBool("DoNormalize",
                    false);  // TODO: add a function to construct general 1-tauH

  auto psis = std::vector<MPS>(dk - 1);

  cpu_time expand_time;
  for (int i = 0; i < dk - 1; ++i) {
    auto args1 = Args("Method=", method, "Cutoff=", truncK, "MaxDim=", maxDim,
                      "Nsweep=", nsw);
    ...
    psis.at(i).noPrime();
    if (donormalize) psis.at(i).normalize();
  ...
  printfln("\nmaxLinkDim after global subspace expansion = %d",
           maxLinkDim(phi));
  printfln("Global subspace expansion: cputime = %s, walltime = %s",
           showtime(sm.time), showtime(sm.wall));
}

void addBasis(MPS& phi, const MPO& H, std::vector<int> const& maxdimK,
              const Args& args0 = Args::global()) {
  auto dk = args0.getInt("KrylovOrd", 2);
  auto method = args0.getString("Method", "DensityMatrix");
  auto nsw = args0.getInt("Nsweep", 2);
  ...
  for (int i = 0; i < dk - 1; ++i) {
    auto args1 =
        Args("Method=", method, "MaxDim=", maxdimK.at(i), "Nsweep=", nsw);
