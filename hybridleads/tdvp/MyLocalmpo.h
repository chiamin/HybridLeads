...
//
#ifndef __ITENSOR_MyLocalMPO_CMC__
#define __ITENSOR_MyLocalMPO_CMC__
...
//
// The MyLocalMPO class projects an MPO
...
class MyLocalMPO {
 public:
  ...
  //
  MyLocalMPO(const MPO& H, const ITensor& LH, const ITensor& RH,
             const Args& args = Args::global());
  ...
  //
  MyLocalMPO(const MPS& Psi, const ITensor& LP, const ITensor& RP,
             const Args& args = Args::global());
  ...
  //
  MyLocalMPO(MPO const& H, ITensor const& LH, int LHlim, ITensor const& RH,
  ...
  ITensor const& L(int i) const { return PH_[i - 1]; }
  ITensor const& R(int i) const { return PH_[i + 1]; }
  void setL(int i, const ITensor& L) { PH_[i - 1] = L; }
  void setR(int i, const ITensor& R) { PH_[i + 1] = R; }

  const MPO& H() const {
    if (Op_ == 0) Error("MyLocalMPO is null or contains an MPS");
    return *Op_;
  }
  ...
  void doWrite(bool val, Args const& args = Args::global()) {
    if (Psi_ != 0)
      Error(
          "Write to disk not yet supported for MyLocalMPO initialized with an "
          "MPS");
  ...
  const ITensor& operator()(int i) const { return PH_.at(i); }
 ...
 private:
  ...
  int LHlim_, RHlim_;  // LHlim_ will be nc-1; RHlim_ will be nc+Nuc
  int nc_;
...
inline MyLocalMPO::MyLocalMPO()
    : Op_(0), LHlim_(-1), RHlim_(-1), nc_(2), Psi_(0) {}

inline MyLocalMPO::MyLocalMPO(const MPO& H, const Args& args)
    : Op_(&H),
      PH_(H.length() + 2),
      LHlim_(0),
      RHlim_(H.length() + 1),
...
inline MyLocalMPO::MyLocalMPO(const MPS& Psi, const Args& args)
    : Op_(0),
      PH_(Psi.length() + 2),
      LHlim_(0),
      RHlim_(Psi.length() + 1),
...
inline MyLocalMPO::MyLocalMPO(const ITensor& LH, const ITensor& RH,
                              const Args& args)
...
inline MyLocalMPO::MyLocalMPO(const MPO& H, const ITensor& LH,
                              const ITensor& RH, const Args& args)
    : Op_(&H),
      PH_(H.length() + 2),
      LHlim_(0),
      RHlim_(H.length() + 1),
...
inline MyLocalMPO::MyLocalMPO(MPS const& Psi, ITensor const& LP,
                              ITensor const& RP, Args const& args)
    : Op_(0),
      PH_(Psi.length() + 2),
      LHlim_(0),
      RHlim_(Psi.length() + 1),
...
inline MyLocalMPO::MyLocalMPO(MPO const& H, ITensor const& LH, int LHlim,
...
void inline MyLocalMPO::product(ITensor const& phi, ITensor& phip) const {
  if (Op_ != 0) {
    ...
    if (nc_ == 2) {
      ...
      if (!L()) {
        if (!R())
          Error("MyLocalMPO: Empty L() and R() in function product");
        else
          othr = R();
      } else {
        othr = L();
    ...
    Error("MyLocalMPO is null");
  }
}

void inline MyLocalMPO::L(int j, ITensor const& nL) {
  if (LHlim_ > j - 1) setLHlim(j - 1);
  PH_[LHlim_] = nL;
}

void inline MyLocalMPO::R(int j, ITensor const& nR) {
  if (RHlim_ < j + 1) setRHlim(j + 1);
  PH_[RHlim_] = nR;
}

inline void MyLocalMPO::position(int b, MPS const& psi) {
  if (!(*this)) Error("MyLocalMPO is null");
...
int inline MyLocalMPO::position() const {
  if (RHlim_ - LHlim_ != (nc_ + 1)) {
    throw ITError("MyLocalMPO position not set");
  }
  return LHlim_ + 1;
}

inline void MyLocalMPO::shift(int j, Direction dir, ITensor const& A) {
  if (!(*this)) Error("MyLocalMPO is null");
...
inline void MyLocalMPO::makeL(MPS const& psi, int k) {
  if (!PH_.empty()) {
    ...
    } else  // normal MPO case
    {
      while (LHlim_ < k) {
        ...
        setLHlim(ll + 1);
        assert(order(PH_.at(ll + 1)) == 3);
      }
    }
  }
}

inline void MyLocalMPO::makeR(MPS const& psi, int k) {
  if (!PH_.empty()) {
    if (Op_ == 0)  // Op is actually an MPS
    {
      while (RHlim_ > k) {
...
void inline MyLocalMPO::setLHlim(int val) {
  if (!do_write_) {
    LHlim_ = val;
    return;
  }
...
void inline MyLocalMPO::setRHlim(int val) {
  if (!do_write_) {
    RHlim_ = val;
    return;
  }
...
void inline MyLocalMPO::initWrite(Args const& args) {
