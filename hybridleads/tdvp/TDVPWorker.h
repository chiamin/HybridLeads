...
void back_propagate(int ha, int b, int numCenter, TimeType t, MPS& psi,
                    const ITensor& phi1, MyLocalMPO& H, Spectrum& spec,
                    const Args& args) {
  auto b1 = (ha == 1 ? b + 1 : b);
...
void TDVPWorker(MPS& psi, MyLocalMPO& H, Cplx t, Sweeps const& sweeps,
  ...
  vector<int> bs;
  if (dir == Fromleft) {
    for (int i = oc; i < N; i++) bs.push_back(i);
    if (numCenter == 1) bs.push_back(N);
  } else {
    for (int i = oc - numCenter + 1; i >= 1; i--) bs.push_back(i);
  }
