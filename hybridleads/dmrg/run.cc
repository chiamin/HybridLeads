...
MPS toMPS(const SiteSet& sites, const ITensor& AL, const Index& iALl,
          const Index& iALr, const Index& iALs, const ITensor& AR,
          const Index& iARl, const Index& iARr, const Index& iARs,
          const ITensor& AC, const Index& iACl, const Index& iACr,
          const Index& iACs, int oc = 1) {
  MPS psi(sites);
  int N = length(psi);
  ...
  for (int i = 2; i < N; i++) {
    ...
    psi.ref(i).replaceInds({iss.at(i), ils.at(i), irs.at(i)},
                           {sites(i), irs.at(i - 1), inew});
    ils.at(i) = irs.at(i - 1);
    irs.at(i) = inew;
  }
  ...
  for (int i = 1; i <= N; i++) {
    ...
    if (i != N) assert(commonIndex(psi(i), psi(i + 1)));
  }
...
int main(int argc, char* argv[]) {
  ...
  // Initialze MPS
  MPS psi = toMPS(sites, AL_L, ISL.il(), prime(ISL.il(), 2), ISL.is(), AR_R,
                  prime(ISR.ir(), 2), ISR.ir(), ISR.is(), AC_L, ISL.il(),
                  ISL.ir(), ISL.is());
  psi.position(1);
  ...
  // Make MPO
  MPO H = single_empurity_mpo(sites, imp_site, t, t_impL, t_impR, muL, muR,
                              mu_imp, V, V_impL, V_impR);
  ...
  // DMRG
  MyObserver<SitesType> myobs(
      sites, psi,
      {"Write", do_write, "out_dir", out_dir, "out_minm", out_minm});
