...
/**
 * @brief For computing (x|[1 - E + |R)(1|]
 *
 */
class SpecialTransferMatrix {
 public:
  SpecialTransferMatrix(const ITensor& AL, const ITensor& R) : _AL(AL), _R(R) {
    assert(order(R) == 2);
  }
 ...
 * @brief
 * @details
 * W should have indices: 1. up site index, 2. down site index, 3. left link
 * index, 4. right link index AL should have indices: 1. site index, 2. left
 * link index, 3. right link index
 *
 *  --------AL----
 *  |       |
 *  L--(b)--W--(a)
 ...
 * @param La0
 * @param args
 * @returns tuple<ITensor, Real>
 *
 * @see PHYSICAL REVIEW B 97, 045145 (2018), Algorithm 6.
 */
template <enumLR dir>
tuple<ITensor, Real> get_LRW(const ITensor& AL, const ITensor& W,
                             const ITensor& R, ITensor& La0, const Args& args) {
  ...
  for (int i = iWa.dim(); i >= 1; i--) {
    if (i != 2) solve_order.push_back(i);
  }
  if constexpr (dir == RIGHT) reverse(solve_order.begin(), solve_order.end());

  auto M = SpecialTransferMatrix(AL, R);

  auto LRW = ITensor(ia, iWb, prime(ia));
  // 1) b == a == (dW or 1), Waa = I  -->  La = I
  for (int i = 1; i <= ia.dim(); i++) LRW.set(i, solve_order.front(), i, 1.);
