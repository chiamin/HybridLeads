...
inline ITensor applyTransfer(const ITensor& x, const ITensor& A1,
                             const ITensor& A2, int pLV = 2) {
  auto re = x * A1;
...
inline ITensor applyTransfer(const ITensor& x, const ITensor& A1,
                             const ITensor& A2, const ITensor& W, int pLV = 2) {
  auto re = x * A1;
  re *= W;
  re *= prime(dag(A2));
  re.prime(-pLV, "Link");
...
inline Real applyT_diff(const ITensor& A1, const ITensor& A2,
                        const ITensor& X) {
...
inline bool check_leading_eigen(const ITensor& A1, const ITensor& A2,
                                const ITensor& X, Real crit = 1e-12) {
...
inline ITensor get_ALR_2(const ITensor& AC, const ITensor& C) {
  ...
  if constexpr (dir == RIGHT) swap(i1, i2);
...
inline ITensor get_LR(const ITensor& C) {
  ...
  if constexpr (dir == RIGHT) swap(ic, iu);
  auto ic2 = prime(ic, 2);
  auto iu2 = prime(iu, 2);
