 ...
 * @throws
 */
bool check_Schur(const ITensor& W, const Index& is, const Index& irow,
                 const Index& icol) {
  ...
  for (int a = dW; a >= 1; a--) {
    ...
    if (a == 2 || a == 1) {
      ...
      if (norm(I - Waa) > 1e-12) throw;
    } else {
      if (norm(Waa) > 1e-12) throw;
    }
...
inline ITensor to_Schur(const ITensor& W1, const Index& is, const Index& irow,
                        const Index& icol) {
  ITensor W;
  int D = irow.dim();
  assert(irow.dim() == icol.dim());
...
/**
 * @brief Retrive the uniform MPO tensor from MPO object.
 *
 * @param H The MPO object.
 * @returns tuple <ITensor,Index,Index,Index> - {W, is, iwl, iwr}.
 * @retval W - The uniform MPO tensor.
 * @retval is - Physical index of MPO tensor W.
 * @retval iwl - Left index of MPO tensor W.
 * @retval iwr - Right index of MPO tensor W.
 * @throws
 */
...
tuple<ITensor, Index, Index, Index> get_MPO_Tensor_Hubbard_spinless(
    Real t, Real mu, const Args& args = Args::global()) {
...
tuple<ITensor, Index, Index, Index> get_MPO_Tensor_XXZ_spin1(
    Real Delta, const Args& args = Args::global()) {
...
void to_inf_mpo(MPO& mpo, Index iL = Index(), Index iR = Index()) {
  ...
  mpo.ref(1) = replaceInds(mpo(2), {is2, prime(is2), i2l, i2r},
                           {is1, prime(is1), iL, i1r});
  ...
  mpo.ref(N) = replaceInds(mpo(N - 1), {isN2, prime(isN2), iN2l, iN2r},
                           {isN, prime(isN), iNl, iR});
  assert(commonIndex(mpo(N - 1), mpo(N)));
}

MPO single_empurity_mpo(const SiteSet& sites, int imp_site, Real t, Real t_impL,
                        Real t_impR, Real muL, Real muR, Real mu_imp, Real V,
                        Real V_impL, Real V_impR) {
  auto N = length(sites);
  AutoMPO ampo(sites);
  for (int i = 1; i <= N; ++i) {
    Real mui, ti, Vi;
    ...
    if (mui != 0.) ampo += -mui, "N", i;
    if (Vi != 0. && i != N) ampo += Vi, "N", i, "N", i + 1;
  }
  auto H = toMPO(ampo);
...
tuple<AutoMPO, int, int> t_mu_V_ampo(const SiteSet& sites, int L_device,
                                     Real t_leadL, Real t_leadR, Real t_device,
                                     Real t_contactL, Real t_contactR,
                                     Real mu_leadL, Real mu_leadR,
                                     Real mu_device, Real V_leadL, Real V_leadR,
                                     Real V_device, Real V_contactL,
                                     Real V_contactR) {
  auto N = length(sites);
...
void ampo_add_mu(AutoMPO& ampo, const vector<tuple<int, Real>>& site_mu,
                 bool verbose = false) {
  if (verbose) cout << "i mu" << endl;
  for (auto [site, mu] : site_mu) {
    if (mu != 0.) {
      ampo += -mu, "N", site;
      if (verbose) cout << site << " " << mu << endl;
    }
  }
}

void ampo_add_tV(AutoMPO& ampo,
                 const vector<tuple<int, int, Real, Real>>& link_t_V,
                 bool verbose = false) {
  if (verbose) cout << "i j  t V" << endl;
