...
using ToLocDict =
    vector<pair<string, int>>;              // ortical index -> {partition, ki}
using SortInfo = tuple<string, int, Real>;  // basis name, orbital index, energ

/**
 * @brief Make dictionary to map between global and local indices.
...
// ---- Recursive functions
void get_sort_info_recursive(const vector<SortInfo>& info) {
}  // Terminate the recursive function calls

template <typename BasisT, typename... Bases>
void get_sort_info_recursive(vector<SortInfo>& info, const BasisT& basis,
                             const Bases&... bases) {
...
vector<SortInfo> sort_by_energy_S_middle_charging(
    ...
    std::initializer_list<LeadBasis> other_chains) {
