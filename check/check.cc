...
#include "Corr.h"
//#include "/home/chiamin/Project/2021/topological_qubit/code/mixedbasis/git/SingleParticle.h"
...
AutoMPO set_H (const SiteSet& sites, int L_lead, int L_device,
               Real t_lead, Real t_device, Real t_contact,
               Real muL, Real muS, Real muR, Real V, bool periodic=false)
{
    int N = length (sites);
    AutoMPO ampo (sites);
    for(int i = 1; i <= N; ++i)
...
AutoMPO set_H_SC (const SiteSet& sites, int L_lead, int L_device,
               Real t_lead, Real t_device, Real t_contact,
               Real muL, Real muS, Real muR, Real V, Real Delta)
{
    int N = length (sites);
    AutoMPO ampo (sites);
    for(int i = 1; i <= N; ++i)
...
int main(int argc, char* argv[])
{
    ...
    auto ampo = set_H (sites, L_lead, L_device, t_lead, t_device, 0., mu_leadL+mu_biasL, mu_device+mu_biasS, mu_leadR+mu_biasR, V);
    auto H = toMPO (ampo);
    ...
    ampo = set_H (sites, L_lead, L_device, t_lead, t_device, t_contact, mu_leadL, mu_device, mu_leadR, V);
    auto Ht = toMPO (ampo);
