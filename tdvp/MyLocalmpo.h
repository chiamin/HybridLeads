...
class MyLocalMPO
    ...
    H() const 
        { 
        if(Op_ == 0)
            Error("MyLocalMPO is null or contains an MPS");
        return *Op_;
        }
    ...
    doWrite(bool val,
            Args const& args = Args::global()) 
        { 
        if(Psi_ != 0) Error("Write to disk not yet supported for MyLocalMPO initialized with an MPS");
...
product(ITensor const& phi, 
    ...
    if(Psi_ != 0)
        ...
        if(nc_ == 2)
            ...
            if(!L())
                {
                if(!R()) Error("MyLocalMPO: Empty L() and R() in function product");
                else othr = R();
                }
            else
                {
