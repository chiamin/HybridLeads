...
inline ITensor to_itdvp_AR (const ITensor& AR, const Index& iAl, const Index& iAr)
{
...
inline ITensor to_itdvp_AL (const ITensor& AL, const Index& iAl, const Index& iAr)
{
...
ITensor get_L (const ITensor& AR, const Index& iAl, const Index& iAr, Real crit=1e-15)
{
    auto il = global::IS.il();
    auto ir = global::IS.ir();
    auto is = global::IS.is();
...
ITensor get_R (const ITensor& AL, const Index& iAl, const Index& iAr, Real crit=1e-15)
{
    auto il = global::IS.il();
    auto is = global::IS.is();
    auto iAs = findIndex (AL, "Site");
               ...
               int& idev_first, int& idev_last)
{
    ...
    = t_mu_V_ampo (sites, L_device,
                   t_lead, t_lead, t_device, t_contact, t_contact,
...
void writeAll (const string& filename,
               ...
               const Index& il, const Index& ir,
               const ITensor& WL, const ITensor& WR,
               const ITensor& AL_left,  const ITensor& AR_left,  const ITensor& AC_left,  const ITensor& C_left,
               const ITensor& La_left,  const ITensor& Ra_left,  const ITensor& LW_left,  const ITensor& RW_left,
               const ITensor& AL_right, const ITensor& AR_right, const ITensor& AC_right, const ITensor& C_right,
               const ITensor& La_right, const ITensor& Ra_right, const ITensor& LW_right, const ITensor& RW_right,
               int step, int idev_first, int idev_last, bool expand, bool expand_next)
{
...
int main(int argc, char* argv[])
{
    ...
    {
        readAll (read_dir+"/"+read_file,
                 psi, H, il, ir, WL, WR,
                 AL_left,  AR_left,  AC_left,  C_left,  La_left,  Ra_left,  LW_left,  RW_left,
                 AL_right, AR_right, AC_right, C_right, La_right, Ra_right, LW_right, RW_right,
                 step, idev_first, idev_last, expand, expand_next);
        auto site_inds = get_site_inds (psi);
        sites = Fermion (site_inds);
    }
    ...
    for(int i = 0; i < time_steps; i++)
    {
        ...
        // Evolve left edge
        tie (en, err, LW_left, RW_left) = itdvp (WL, AL_left, AR_left, AC_left, C_left, La_left, Ra_left, 1_i*dt, args_itdvp);
        PH->L (1, LW_left);
        ...
        tie (en, err, LW_right, RW_right) = itdvp (WR, AL_right, AR_right, AC_right, C_right, La_right, Ra_right, 1_i*dt, args_itdvp);
        PH->R (N, RW_right);
